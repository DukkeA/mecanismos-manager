...
export async function createAccount(actor: Actor, raw: unknown) {
  ...
  const input = z
    .object({
      requestId: z.uuid(),
      name: z.enum(accountNames),
      openingBalance: money,
    })
    .parse(raw);
  return once(actor, input.requestId, "ACCOUNT_CREATED", input, async (tx) => {
    if (await tx.moneyAccount.findUnique({ where: { name: input.name } }))
      throw new DomainError("Esta cuenta ya existe.");
    ...
    await tx.auditEvent.create({
      data: {
        ...
        details: { openingBalance: input.openingBalance },
      },
    });
    ...
    async (tx) => {
      ...
      await tx.auditEvent.create({
        data: {
          ...
          details: { category: input.category, period: input.period },
        },
      });
...
export async function recordCash(actor: Actor, raw: unknown) {
  ...
  return once(actor, input.requestId, "CASH_RECORDED", input, async (tx) => {
    ...
    if (input.obligationId) {
      if (input.kind !== "EXPENSE_PAYMENT")
        throw new DomainError(
          "Solo los pagos de gasto pueden abonar una obligación.",
        );
      const obligation = await tx.obligation.findUniqueOrThrow({
        where: { id: input.obligationId },
        include: { entries: true },
      });
      if (obligation.salaryPeriod)
        throw new DomainError(
          "Registra los salarios desde Equipo → Pago del mes.",
        );
      if (obligation.category === "PAYROLL")
        requirePermission(actor.role, "payroll:read");
      const paid = obligation.entries.reduce(
        (sum, e) =>
          sum.plus(
            e.direction === "OUT"
              ? e.amount.toString()
      ...
      if (paid.plus(amount).gt(obligation.amount.toString()))
        throw new DomainError(
          "El pago supera el saldo pendiente de la obligación.",
        );
    }
    const balance = new Decimal(account.balance.toString()).plus(
      direction === "IN" ? amount : amount.negated(),
    );
    if (balance.lt(0))
      throw new DomainError(
        "La cuenta no tiene saldo suficiente. Revisa entradas y saldo inicial.",
      );
    ...
    await tx.auditEvent.create({
      data: {
        ...
        details: { kind: input.kind, accountId: account.id },
      },
    });
    return { id: entry.id };
  });
...
export async function reverseCash(actor: Actor, raw: unknown) {
  ...
  return once(actor, input.requestId, "CASH_REVERSED", input, async (tx) => {
    ...
    if (
      ...
      (await tx.cashEntry.findUnique({ where: { reversalOfId: original.id } }))
    )
      throw new DomainError("Ese movimiento no admite otra reversión.");
    await assertOpenCash(tx, original.accountId, input.occurredOn);
    if (
      original.kind === "SALARY_PAYMENT" ||
      (await tx.payrollPayment.findFirst({ where: { entryId: original.id } }))
    )
      throw new DomainError(
        "Revierte el pago desde Equipo → Pago del mes para actualizar sus descuentos.",
      );
    if (original.kind === "SALARY_ADVANCE")
      throw new DomainError(
        "Anula el anticipo desde Equipo para actualizar también sus cuotas.",
      );
    ...
    if (payment?.refunds.some((r) => !r.entry.reversal))
      throw new DomainError(
        "Revierte las devoluciones de este anticipo antes de revertir su cobro.",
      );
    if (payment)
    ...
    const next = new Decimal(account.balance.toString()).plus(
      direction === "IN"
        ...
        : new Decimal(original.amount.toString()).negated(),
    );
    if (next.lt(0))
      throw new DomainError("No hay saldo para revertir esta entrada.");
    const entry = await tx.cashEntry.create({
      data: {
