...
const definition: Record<HubResource, Prisma.Sql> = {
 ...
 LEFT JOIN LATERAL(SELECT sum(e.minutes) minutes,sum(CASE WHEN r.id IS NULL THEN e.minutes ELSE 0 END) missing,sum(e.minutes*r."hourlyCost"/60.0) cost FROM workshop."TimeEntry" e JOIN workshop."Task" task ON task.id=e."taskId" LEFT JOIN LATERAL(SELECT id,"hourlyCost" FROM workshop."LaborRate" WHERE "memberId"=e."memberId" AND "effectiveOn"<=e."workedOn" ORDER BY "effectiveOn" DESC LIMIT 1)r ON true WHERE task."orderId"=o.id)t ON true
