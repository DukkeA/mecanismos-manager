...
Disponible para pruebas locales: cotizaciones, ventas, cartera, compras, inventario, órdenes, tareas, garantías, unidades propias, costos y control de caja. Incluye 63 modelos privados, 22 migraciones locales y cinco diagramas Archify. Los datos ficticios persisten en Docker. Con los accesos locales habilitados, `/login` permite elegir administración, oficina o mecánico; `/` valida la sesión y `/demo` redirige a este flujo.
...
4. `pnpm db:generate`, `pnpm db:seed:local`, `pnpm db:storage:local`, `pnpm db:migrate:photos:local`, `pnpm db:seed:commerce`, `pnpm db:seed:control`, `pnpm db:seed:team`, `pnpm db:seed:attendance`, `pnpm db:seed:benefits`, `pnpm db:seed:payroll` y `pnpm db:seed:supports`.
