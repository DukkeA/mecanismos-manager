...
let accountId:string;let obligationId:string;let payrollId:string;
...
afterAll(async()=>{
  ...
  await db().obligation.deleteMany({where:{id:{in:[obligationId,payrollId].filter(Boolean)}}});
...
describe("cash ledger against PostgreSQL",()=>{
  it("prevents simultaneous payments from exceeding an obligation and reverses once",async()=>{
    ...
    const paid=(await getOperations(admin)).obligations.find(o=>o.id===obligationId)!;
    expect(paid.paid).toBe("75.00");
    const success=results.find(r=>r.status==="fulfilled") as PromiseFulfilledResult<{id:string}>;
    const reversal={requestId:randomUUID(),entryId:success.value.id,reason:"Reverse integration payment",occurredOn:"2026-09-09"};
    expect(await reverseCash(admin,reversal)).toEqual(await reverseCash(admin,reversal));
    expect((await getOperations(admin)).obligations.find(o=>o.id===obligationId)!.paid).toBe("0.00");
    expect((await db().moneyAccount.findUniqueOrThrow({where:{id:accountId}})).balance.toString()).toBe("1000");
  });
  it("rejects an overdraft and replays the same receipt without duplicating cash",async()=>{
    await expect(recordCash(office,{...payment("1001"),obligationId:undefined})).rejects.toThrow("saldo suficiente");
    const input={...payment("25"),obligationId:undefined,kind:"CUSTOMER_PAYMENT"};
    const [a,b]=await Promise.all([recordCash(office,input),recordCash(office,input)]);
    expect(a).toEqual(b);
    expect((await db().moneyAccount.findUniqueOrThrow({where:{id:accountId}})).balance.toString()).toBe("1025");
  });
  it("keeps payroll out of office responses and disallows office payment or borrowing",async()=>{
    await recordCash(admin,{...payment("50"),obligationId:payrollId});
    const view=await getOperations(office);
    expect(view.obligations.some(o=>o.id===payrollId)).toBe(false);
    expect(view.cashEntries.some(e=>e.obligationId===payrollId)).toBe(false);
    await expect(recordCash(office,{...payment("50"),obligationId:payrollId})).rejects.toThrow("permiso");
    await expect(recordCash(office,{...payment("50"),obligationId:undefined,kind:"LOAN_RECEIVED"})).rejects.toThrow("administración");
  });
  it("does not grant runtime mutation of cash history",async()=>{
    const result=await db().$queryRaw<{allowed:boolean}[]>`SELECT has_table_privilege('workshop_runtime','workshop."CashEntry"','UPDATE') AS allowed`;
