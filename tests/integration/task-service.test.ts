...
const input = {
  ...
  minutes: 45,
  workedOn: "2026-09-09",
  note: "Prueba de presión en banco",
};

beforeAll(async () => {
  await db().member.createMany({
    data: [memberId, otherId].map((id) => ({
      id,
...
afterAll(async () => {
  await db().auditEvent.deleteMany({
    where: { actorId: { in: [memberId, otherId] } },
  });
  ...
  await cleanupActivity(
    (
      await db().member.findMany({ where: { id: { in: [memberId, otherId] } } })
    ).map((m) => m.id),
  );
