...
beforeAll(async () => {
  await db().member.createMany({
    data: [admin, mechanic, outsider].map((a) => ({
      ...a,
  ...
  await db().catalogItem.create({
    data: { id: itemId, code: itemId, name: "Test part", kind: "PART" },
  });
});
afterAll(async () => {
  const order = orderId
  ...
  if (order) {
    await db().orderAsset.deleteMany({ where: { orderId } });
    await db().workOrder.delete({ where: { id: orderId } });
    await db().asset.deleteMany({
      where: { id: { in: order.assets.map((a) => a.assetId) } },
    });
    if (order.customerId)
...
describe("operational transactions against PostgreSQL", () => {
  it("receives once on concurrent retries", async () => {
    const input = {
      ...
      title: "Test injection pump",
      customer: "Test customer",
      reference: "TEST",
      dueAt: "2026-09-20",
      kind: "COMPONENT",
      problem: "Needs pressure test",
      locationId,
    };
    const [a, b] = await Promise.all([
      receiveOrder(admin, input),
      receiveOrder(admin, input),
    ]);
    expect(a).toEqual(b);
    orderId = a.id as string;
    expect(
      (
  ...
  it("enforces assignments and versioned lifecycle", async () => {
    ...
    await expect(
      changeTaskStatus(outsider, { taskId, status: "DONE" }),
  ...
  it("values weighted material cost and prevents concurrent overdraw", async () => {
    for (const cost of ["100", "300"])
      await moveStock(admin, {
        requestId: randomUUID(),
        itemId,
        locationId,
        condition: "NEW",
    ...
    expect(balance.materialCost.toString()).toBe("1000");
    const successful = outcomes.find(
      (o) => o.status === "fulfilled",
    ) as PromiseFulfilledResult<{ id: string }>;
    const reverse = {
      requestId: randomUUID(),
      movementId: successful.value.id,
      reason: "Reverse test consumption",
    };
    expect(await reverseMovement(admin, reverse)).toEqual(
      await reverseMovement(admin, reverse),
    );
    expect(
      (
        await db().stockBalance.findFirstOrThrow({ where: { itemId } })
      ).quantity.toString(),
    ).toBe("20");
  });
  it("does not expose or mutate inventory as a mechanic", async () => {
    ...
    expect(view.members.every((m) => !("email" in m) && !("role" in m))).toBe(
      true,
    );
    await expect(
      moveStock(mechanic, {
  ...
  it("grants runtime only append access to ledgers", async () => {
    const result = await db().$queryRaw<
      { allowed: boolean }[]
    >`SELECT has_table_privilege('workshop_runtime','workshop."StockMovement"','UPDATE') AS allowed`;
