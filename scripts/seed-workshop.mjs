...
try {
 ...
 if(!seeded.rowCount){
  ...
  const openings=[2500000,22000000,0,0],balances=openings.map(v=>new Decimal(v));
  for(const [i,name] of ['Oficina','Bodega','Caja Menor Bodega','Caja Menor Oficina'].entries())await insert('MoneyAccount',{id:id('account'+i),name,openingBalance:openings[i],balance:openings[i]});
